...
static void	search_and_exec(t_shell *shell, char **argv, char **envp)
{
	...
	if (!paths)
		free_argv_envp_exit(shell, argv, envp, 1);
	...
	if (path)
		execute_valid_path(shell, path, argv, envp);
...
void	execute_command(t_shell *shell, t_cmd *cmd)
{
	...
	if (!argv)
		exit_child(shell, 1);
	if (!argv[0])
		free_argv_envp_exit(shell, argv, NULL, 0);
	envp = env_to_array(shell->env_list);
	if (!envp)
		free_argv_envp_exit(shell, argv, NULL, 1);
	if (argv[0][0] == '\0')
	{
		print_cmd_error(argv[0], ": command not found\n");
		free_argv_envp_exit(shell, argv, envp, 127);
	...
	if (ft_strchr(argv[0], '/'))
		exec_absolute_path(shell, argv, envp);
	else
